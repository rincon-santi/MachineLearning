...
def lda(X,t):
    ...
    F = [(s[0]-s[1]),
         ...
         (-s[1]*pow(m[0],2) +
           s[0]*pow(m[1],2)-2*s[0] *
           s[1]*math.log(p[1]/math.sqrt(s[1])) +
           2*s[0]*s[1]*math.log(p[0]/math.sqrt(s[0])))
        ]
    c = np.roots([F[0],F[1],F[2]])
    F_prima = [2*F[0], F[1]]
    if math.fabs(c[0]*F_prima[0]-F_prima[1]) <= math.fabs(c[1]*F_prima[0]-F_prima[1]):
...
fig, ax = plt.subplots()
fig.canvas.set_window_title('Practica 1')
ax.set_xlim([-20,20])
ax.set_ylim([-20,20])
...
slid = Slider(axslid, 'Classes', 2, 5, valfmt='%d', valinit=2)
